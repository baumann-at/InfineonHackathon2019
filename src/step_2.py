...
def getToSign():
  try:
    print('')
    print('GET DATA TO SIGN ...');
    url = 'https://_REMOVED_CB_/ih19/gettosign.php'
...
print('-------------------------------------------------------------')
print('Step 2: Sign data provided by blockchain-app and send it back');
print('-------------------------------------------------------------')
...
for x in range(1):
  try:
    ...
    toSign = getToSign()
    hash = hashlib.sha256(toSign.encode()).digest()
    hash = bytes(hash)

    print('Text to sign: ' + toSign)
    (global_counter, counter, signature) = generate_signature(reader, key_id, hash)
    print('Signature: ' + signature.hex())

    name = input('Enter your name (optionally): ')
  ...
  try:
    ...
    payload = {'Card': card_id.hex(), 'PubKey': key.hex(), 'toSign': toSign, 'Signature': signature.hex(), 'Name': name}
